...
for split in splits:
    ...
    output_json_path = f'./pocket_structures_{split}.json'
    invalid_pdb_ids_path = f'./invalid_pdb_ids_{split}.json'
    protein_dict = {}
    ...
    with open(csv_file_path, newline="") as csvfile:
        ...
        for row in reader:
            ...
            print(i)
            try:
                pdb_id = row[0].split("_")[1]
            except IndexError:
                print(f"Error: Invalid PDB ID format: {row[0].split('_')[1]}")
                continue

            try:
                invalid_pdb_id = process_invalid_pdb_data(pdb_id, pdb_data_dir)
                if invalid_pdb_id:
                    invalid_pdb_ids_list.append(invalid_pdb_id)
                    continue

                protein_data = process_pdb_file(pdb_id, pdb_data_dir)
                for pdb_id, entry in protein_data.items():
                    protein_dict[pdb_id] = entry  # 累积蛋白质数据
